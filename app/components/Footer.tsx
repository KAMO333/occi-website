...
export default function Footer() {
  return (
    <section id="contact" className="border-t border-white/8 bg-[#0a1428] py-20">
      <div className="max-w-6xl mx-auto px-6">
        <div className="grid md:grid-cols-3 gap-8 mb-16">
          <FadeIn>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-8 h-8 rounded bg-[#2B7BC8] flex items-center justify-center"><span style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 13 }}>OC</span></div>
          ...
          <FadeIn delay={0.2}>
            ...
            <div className="space-y-2 text-sm">
              {["Services", "About", "Contact"].map((link) => (
                <a key={link} href={`#${link.toLowerCase()}`} className="block text-white/50 hover:text-[#4A9FE0] transition-colors">{link}</a>
              ))}
            </div>
          </FadeIn>
        </div>
